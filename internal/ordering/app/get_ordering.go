...
func (s *OrderingService) CheckOrderByProductIdAct(ctx context.Context, request domain.CheckOrderByProductIdRequest) (domain.CheckOrderByProductIdResponse, error) {
	if request.ProductId <= 0 {
		if logCtx, ok := shared.LogContextFrom(ctx); ok {
			shared.ZapLogger("warn", "Ordering Get Service Log", logCtx.Service, logCtx.RequestID, logCtx.URL, logCtx.RequestBody, []byte("product id must be greater than zero"))
		}
	...
	if s.repo == nil {
		if logCtx, ok := shared.LogContextFrom(ctx); ok {
			shared.ZapLogger("warn", "Ordering Get Service Log", logCtx.Service, logCtx.RequestID, logCtx.URL, logCtx.RequestBody, []byte("ordering repository is not configured"))
		}
